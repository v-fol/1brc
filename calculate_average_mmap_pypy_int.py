...
def process_chunk(chunk) -> dict:
    ...
    def parse_float_to_int(x: bytes) -> int:
        # Parse sign
        if x[0] == 45:  # ASCII for "-"
            sign = -1
            idx = 1
        ...
        with mmap.mmap(
            ...
            for subchunk in subchunks:
                ...
                while index < subchunk:
                    ...
                    temp = parse_float_to_int(buffer[semicolon + 1: newline])
...
def main(filename: str, cpu_count: int = cpu_count()):
    ...
    answer = ", ".join(
        f"{station_name.decode()}={station[0] * 0.1:.2f}/{(station[2]* 0.1)/station[3]:.2f}/{station[1] * 0.1:.2f}"
